...
const COMPANY_META = {
  'Credit Karma / Intuit': { color: '#00d4ff', dot: 'bg-accent' },
  'Diadem Capital':        { color: '#7c3aed', dot: 'bg-violet' },
  'SJSU King Library':     { color: '#10b981', dot: 'bg-emerald-500' },
  'Goldman Sachs':         { color: '#f59e0b', dot: 'bg-amber-500' },
}
...
export default function Experience({ data }) {
  ...
  const meta  = COMPANY_META[job?.company] ?? { color: '#00d4ff', dot: 'bg-accent' }

  return (
    <SectionWrapper id="experience" className="bg-[#eef1ff] dark:bg-[#080c1e]">
      ...
      <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-0 md:gap-10">
              ...
              <button
                ...
                className={[
                  ...
                  isActive
                    ...
                    : 'border-transparent text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-white/50 dark:hover:bg-white/[0.03] hover:border-slate-300 dark:hover:border-white/20',
                ].join(' ')}
              >
        ...
        <div ref={panelRef} role="tabpanel" className="flex-1 pt-7 md:pt-0 min-h-[400px]">
          <div className="mb-7">
            ...
            <p className="mt-1.5 text-xs font-mono text-slate-400 dark:text-slate-600 tracking-wide">
          ...
          <ul className="space-y-4">
            {job?.bullets?.map((bullet, i) => (
              <li key={i} className="exp-bullet flex gap-3 text-[14px] text-slate-600 dark:text-slate-400 leading-relaxed">
